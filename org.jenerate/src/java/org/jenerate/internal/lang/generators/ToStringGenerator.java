...
public final class ToStringGenerator implements ILangGenerator {
	 ...
	 * 
	 * @see
	 * org.jenerate.internal.lang.generators.ILangGenerator#generate(org.eclipse
	 * .swt.widgets.Shell, org.eclipse.jdt.core.IType)
	 */
	public void generate(Shell parentShell, IType objectClass) {
		...
		try {
			...
			ToStringDialog dialog = new ToStringDialog(parentShell,
					...
					excludedMethods,
					!JavaUtils.isToStringConcreteInSuperclass(objectClass));
			...
			if (returnCode == Window.OK) {
				...
				boolean useGettersInsteadOfFields = dialog
			...
			MessageDialog.openError(parentShell, "Method Generation Failed",
					e.getMessage());
		}
	...
	private void generateToString(final Shell parentShell,
			...
			final boolean useGettersInsteadOfFields) throws PartInitException,
			JavaModelException {
		...
		objectClass.getCompilationUnit()
				.createImport(
						CommonsLangLibraryUtils.getToStringBuilderLibrary(),
						null, null);
	...
	private String getStyleConstantAndAddImport(final String style,
		...
		if (!style.equals(CommonsLangLibraryUtils
				.getToStringStyleLibraryDefaultStyle()) && !style.equals("")) {
			...
			if (lastDot != -1 && lastDot != (style.length() - 1)) {
				...
				if (lastDot2 != (styleClass.length() - 1)) {
					...
					styleConstant = style.substring(lastDot2 + 1,
							style.length());
					if (lastDot2 != -1) {
	...
	private String createBuilderString(final IField[] checkedFields,
		...
		for (int i = 0; i < checkedFields.length; i++) {
			...
			content.append("\", ");
			content.append(JavaUtils.generateFieldAccessor(checkedFields[i],
					useGettersInsteadOfFields));
			content.append(")");
		}
	...
	private static class ToStringDialog extends OrderableFieldDialog {
		 ...
		 * 
		 * @see
		 * org.jenerate.internal.ui.dialogs.FieldDialog#createOptionComposite
